...
rlm(formula = hwy ~ displ, data = mpg, method = "bgd")
```


### with boldDriver
```{r bgdBoldDriver, message=FALSE, warning=FALSE}
rlm(formula = hwy ~ displ, data = mpg, method = "bgd", boldDriver = TRUE)
```
...
rlm(formula = hwy ~ displ, data = mpg, method = "sgd")
```


### with Adagrad
```{r sgdAda, message=FALSE, warning=FALSE}
rlm(formula = hwy ~ displ, data = mpg, method = "sgd", AdaGrad = TRUE)
```

## rlm for Mini-batch Gradient Descent
```{r mbgd, message=FALSE, warning=FALSE}
rlm(formula = hwy ~ displ, data = mpg, method = "mini-bgd")
```


## rlm for Coordinate Descent
```{r cd, message=FALSE, warning=FALSE}
rlm(formula = hwy ~ displ, data = mpg, method = "cd")
```
...
bgd <- rlm(formula = hwy ~ displ, data = mpg, method = "bgd")
coef(bgd)
```
...
rlm(formula = hwy ~ displ, data = mpg, method = "bgd", alpha=0.2)
```
...
rlm(formula = hwy ~ displ, data = mpg, method = "bgd", alpha=0.2, max.iter = 500)
```
...
rlm(formula = hwy ~ displ, data = mpg, method = "bgd", precision = 0.0002)
```
